<template>
  <PageSubtitle title="Edit trailer" />
  <div class="d-flex flex-column">
    ...
    <button @click="validateTrailer" class="btn btn-primary align-self-end">Edit trailer</button>
  </div>
  ...
  <base-modal id="dataModal" title="Trailer">
    ...
    <template #footer>
      <div class="d-flex justify-content-between align-items-center w-100">
        <div>Do you want to edit this trailer?</div>
        <div class="d-flex">
          <button class="btn btn-primary me-2" @click="editTrailer">Yes</button>
    ...
    async editTrailer() {
      ...
      await update(ref(database, `games/${this.gameId}`), {
        ...
        .then(() => {
          ...
          this.serverResponse = 'You edited the trailer successfully!'
          this.openServerResponseModal()
        })
        .catch((error) => {
          // The write failed...
          this.uploading = false
          this.serverResponse = 'Ups something went wrong... You did not edit the trailer.'
          this.openServerResponseModal()
        })
    },
